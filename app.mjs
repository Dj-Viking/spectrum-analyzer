...
export function updateCanvas(ctx, type, meterParams) {
...
export function app(appModule, utilsModule, meterNodeModule) {
    const startbtn = document.querySelector("#start-audio");
    const volumeInput = document.querySelector("#volume-input");
    const volumeLevel = document.querySelector("#level");
    const volumeEl = document.querySelector("#volume");
    const smoothingEl = document.querySelector("#smoothing-input");
    const smoothingSpan = document.querySelector("#smoothing");
    const barheightEl = document.querySelector("#barheight-input");
    const barheightSpan = document.querySelector("#barheight");
    const colorvariationEl = document.querySelector("#colorvariation-input");
    const colorvariationSpan = document.querySelector("#colorvariation");
    ...
    function spectrumDraw() {
        if (analyserNode) {
            ...
            for (let i = 0; i < analyserBufferLength; i++) {
                ...
                spectrumCtx.fillRect(barPosX, finalBarHeight, barWidth, barHeight / 2);
                barPosX += barWidth + 1;
            }
        }
        window.requestAnimationFrame((ts) => frameHandler(spectrumDraw)(ts));
    }
    function frameHandler(work) {
        work();
    ...
    startbtn.onclick = () => {
        ...
        (async () => {
            window.navigator.getUserMedia({
                ...
                // analyserNode.
                meterNode = new meterNodeModule.MeterNode(audioCtx, FRAME_RATE_IN_MS, appModule, utilsModule, canvas, volumeEl, frameHandler);
